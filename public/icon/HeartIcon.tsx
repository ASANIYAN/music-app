...
const HeartIcon = ({color, like, handleLike} : Prop) => {

    return (
        <>
            <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg" onClick={handleLike}>
                ...
                fill={`${like ? `${color}` : ""}`}
                stroke={`${like ? `${color}` : "#FACD66"}`} 
                strokeWidth="0.5625" 
                strokeLinecap="round" 
                strokeLinejoin="round"/>
                <path opacity="0.4" d="M12 5.02501C12.8025 5.28451 13.3695 6.00076 13.4377 6.84151" stroke={`${like ? `${color}` : "#FACD66"}`} strokeWidth="0.5625" strokeLinecap="round" strokeLinejoin="round"/>
            </svg>
        </>
    );
}
